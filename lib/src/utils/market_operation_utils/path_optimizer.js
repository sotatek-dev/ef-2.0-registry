...
function findOptimalPathAsync(side, fills, targetInput, runLimit = Math.pow(2, 8), opts = path_1.DEFAULT_PATH_PENALTY_OPTS) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!optimalPath.isComplete()) {
            ...
            const fcxFills = optimalPath.fills.filter(fill => fill.source === types_2.ERC20BridgeSource.FCX);
            if (fcxFills.length > 0 && iocInput.gt(0)) {
                ...
                const fillData = newFill.fillData;
                fillData.fillableMakerAmount = side === types_1.MarketOperation.Sell ? newFill.output : newFill.input;
                fillData.order.makerAmount = side === types_1.MarketOperation.Sell ? newFill.output : newFill.input;
                fillData.fillableTakerAmount = side === types_1.MarketOperation.Sell ? newFill.input : newFill.output;
                fillData.order.takerAmount = side === types_1.MarketOperation.Sell ? newFill.input : newFill.output;
                fillData.order.salt = new utils_1.BigNumber(1); // mark as IoC
                newFill.fillData = fillData;
                // bscSellTokenBalance logic
                const bscFilledBalance = optimalPath.fills.reduce((prev, cur) => {
                ...
                if (side === types_1.MarketOperation.Sell) {
                    ...
                    newFill.input = utils_1.BigNumber.min(newFill.input, remainBscBalance);
                    newFill.output = newFill.input
                        .div(oldInput)
                        .times(newFill.output)
                        .integerValue(utils_1.BigNumber.ROUND_DOWN);
                }
                else if (side === types_1.MarketOperation.Buy) {
                    const oldOutput = newFill.output;
                    newFill.output = utils_1.BigNumber.min(newFill.output, remainBscBalance);
            ...
            const xlmFills = optimalPath.fills.filter(fill => fill.source === types_2.ERC20BridgeSource.XLM);
            if (xlmFills.length > 0 && iocInput.gt(0)) {
                ...
                const xlmFilledBalance = optimalPath.fills.reduce((prev, cur) => {
                    if (cur.source !== types_2.ERC20BridgeSource.XLM) {
                        return prev;
                    }
                    return side === types_1.MarketOperation.Sell ? prev.plus(cur.input) : prev.plus(cur.output);
                }, new utils_1.BigNumber(0));
                const xlmSellTokenBalance = opts.xlmSellTokenBalance || constants_1.POSITIVE_INF;
                const remainXlmBalance = xlmSellTokenBalance.minus(xlmFilledBalance);
                if (side === types_1.MarketOperation.Sell) {
                    const oldInput = newFill.input;
                    newFill.input = utils_1.BigNumber.min(newFill.input, remainXlmBalance);
                    newFill.output = newFill.input
                        .div(oldInput)
                        .times(newFill.output)
                        .integerValue(utils_1.BigNumber.ROUND_DOWN);
                    ...
                    const oldOutput = newFill.output;
                    newFill.output = utils_1.BigNumber.min(newFill.output, remainXlmBalance);
                    newFill.input = newFill.output
                        .div(oldOutput)
                        .times(newFill.input)
                        .integerValue(utils_1.BigNumber.ROUND_DOWN);
