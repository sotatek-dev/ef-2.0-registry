...
class SamplerOperations {
    ...
    getSellQuotes(sources, makerToken, takerToken, takerFillAmounts, autoRouteOpts = {
        poolType: [],
        dmmPoolAddresses: [],
        path: []
    }) {
        const subOps = this._getSellQuoteOperations(sources, makerToken, takerToken, takerFillAmounts, this.tokenAdjacencyGraph, autoRouteOpts);
        return this._createBatch(subOps, (samples) => {
            return subOps.map((op, i) => {
    ...
    getBuyQuotes(sources, makerToken, takerToken, makerFillAmounts, autoRouteOpts = {
        poolType: [],
        dmmPoolAddresses: [],
        path: []
    }) {
        const subOps = this._getBuyQuoteOperations(sources, makerToken, takerToken, makerFillAmounts, autoRouteOpts);
        return this._createBatch(subOps, (samples) => {
            return subOps.map((op, i) => {
                return samples[i].map((output, j) => ({
                    source: op.source,
    ...
    _getSellQuoteOperations(sources, makerToken, takerToken, takerFillAmounts, tokenAdjacencyGraph = this.tokenAdjacencyGraph, autoRouteOpts = {
        poolType: [],
        dmmPoolAddresses: [],
        path: []
    }) {
        ...
        const allOps = _.flatten(_sources.map((source) => {
            ...
            switch (source) {
                ...
                case types_1.ERC20BridgeSource.AutoRoute:
                    ...
                    return this.getAutoRouteSellQuotes(autoRouter, takerFillAmounts, autoRouteOpts);
