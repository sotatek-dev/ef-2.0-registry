...
exports.getRandomSignature = exports.getTotalIn = exports.getTotalOut = exports.getRandomAmount = exports.generatePseudoRandomSalt = exports.baseUnitAmount = void 0;
...
function getTotalOut(arr) {
    let total = new utils_1.BigNumber(0);
    if (!arr) {
        return total;
    }
    for (let i = 0; i < arr.length; i++) {
        const item = arr[i];
        total = total.plus(item.output);
    }
    return total;
}
exports.getTotalOut = getTotalOut;
function getTotalIn(arr) {
    let total = new utils_1.BigNumber(0);
    if (!arr) {
        return total;
    }
    for (let i = 0; i < arr.length; i++) {
        const item = arr[i];
        total = total.plus(item.input);
    }
    return total;
}
exports.getTotalIn = getTotalIn;
